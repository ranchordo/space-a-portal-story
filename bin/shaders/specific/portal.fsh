...
void main() {
	...
	if((textureUse&1)>0) {
		fcol=fcol*texture2D(albedo,texCoords);
	}
	float squeeze=0.25;
	...
	FragColor=vec4(noiseval*fcol.xyz,1.0+block*(fcol.w-1.0));//vec4(noiseval*fcol.xyz,1.0+block*(fcol.w-1.0));//vec4(((normal/2.0)+0.5)*0.15,1);
}
...
public class Camera {
	...
	public void rotation(Float p_, Float y_, Float r_) {
		Quat4f rotq=LeptonUtil.noPool(LeptonUtil.AxisAngle(new AxisAngle4f()));
		Matrix4f rotm=new Matrix4f(rotq,new Vector3f(0,0,0),1.0f);
		if(p_!=null) {rotm.mul(new Matrix4f(LeptonUtil.noPool(LeptonUtil.AxisAngle(new AxisAngle4f(1,0,0,p_))),new Vector3f(0,0,0),1.0f));}
		if(y_!=null) {rotm.mul(new Matrix4f(LeptonUtil.noPool(LeptonUtil.AxisAngle(new AxisAngle4f(0,1,0,y_))),new Vector3f(0,0,0),1.0f));}
		if(r_!=null) {rotm.mul(new Matrix4f(LeptonUtil.noPool(LeptonUtil.AxisAngle(new AxisAngle4f(0,0,1,r_))),new Vector3f(0,0,0),1.0f));}
		rot=LeptonUtil.noPool(LeptonUtil.Quat(new Transform(rotm).getRotation(new Quat4f())));
		rotmod();
	...
	public void transformRotation(Matrix4f tr) {
		Quat4f rotq=LeptonUtil.noPool(LeptonUtil.AxisAngle(rot));
	...
	public Transform getTransform() {
		ret.set(new Matrix4f(LeptonUtil.noPool(LeptonUtil.AxisAngle(rot)),pos,1.0f));
		return ret;
	}
	...
	public void rotate(Float p_, Float y_, Float r_) {
		...
		if(y_!=null) {rotm.mul(new Matrix4f(LeptonUtil.noPool(LeptonUtil.AxisAngle(new AxisAngle4f(0,1,0,y_))),new Vector3f(0,0,0),1.0f));}
		if(r_!=null) {rotm.mul(new Matrix4f(LeptonUtil.noPool(LeptonUtil.AxisAngle(new AxisAngle4f(0,0,1,r_))),new Vector3f(0,0,0),1.0f));}
		rotm.mul(rotm,new Matrix4f(LeptonUtil.noPool(LeptonUtil.AxisAngle(rot)),new Vector3f(0,0,0),1.0f));
		rot=LeptonUtil.noPool(LeptonUtil.Quat(new Transform(rotm).getRotation(new Quat4f())));
		rotmod();
		this.rot_out_aa.set(this.rot.x,this.rot.y,this.rot.z,this.rot.angle);
		updateEuler();
	...
	private void rotmod() {
		...
		rot.set(this.rot.x/rotm,this.rot.y/rotm,this.rot.z/rotm,LeptonUtil.mod(this.rot.angle,(float)Math.PI*2.0f));
	}
